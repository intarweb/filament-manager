...
export default function FilamentSyncSection({ isCloudConnected }: { isCloudConnected: boolean }) {
  ...
  const handleToggle = () => {
    settingsMut.mutate({ enabled: !syncStatus.enabled, direction: syncStatus.direction })
  }
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          disabled={busy || !isCloudConnected}
          className={`relative inline-flex h-5 w-9 shrink-0 rounded-full border-2 border-transparent transition-colors focus:outline-none
            ${syncStatus.enabled && isCloudConnected ? 'bg-blue-500' : 'bg-gray-600'}
            ${!isCloudConnected ? 'opacity-40 cursor-not-allowed' : 'cursor-pointer'}`}
          title={!isCloudConnected ? t('settings.filamentSync.requiresCloud') : undefined}
        >
          <span className={`pointer-events-none inline-block h-4 w-4 rounded-full bg-white shadow transition-transform
            ${syncStatus.enabled && isCloudConnected ? 'translate-x-4' : 'translate-x-0'}`} />
        </button>
      </div>

      {!isCloudConnected && (
        <p className="text-xs text-yellow-500">{t('settings.filamentSync.requiresCloud')}</p>
      )}

      {syncStatus.enabled && isCloudConnected && (
        <>
          {/* Direction selector */}
          <div>
            <label className="label">{t('settings.filamentSync.direction')}</label>
                ...
                <button
                  ...
                  className={`px-3 py-1 rounded text-xs border transition-colors
                    ${syncStatus.direction === dir
                      ...
                      : 'bg-surface-2 border-gray-600 text-gray-400 hover:border-gray-400'}`}
                >
